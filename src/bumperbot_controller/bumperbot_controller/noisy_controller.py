...
class NoisyController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Using wheel_radius %d" % self.wheel_radius_)
        self.get_logger().info("Using wheel_separation %d" % self.wheel_separation_)
    ...
    def jointCallback(self, msg):
        ...
        # update of support variables 
        self.left_wheel_prev_pos_ = msg.position[1]
        self.right_wheel_prev_pos_ = msg.position[0]
        self.prev_time_ = Time.from_msg(msg.header.stamp) 

        fi_left = dp_left / (dt.nanoseconds / S_TO_NS)     # rad/sec
        fi_right = dp_right / (dt.nanoseconds / S_TO_NS)   # rad/sec
        
        # Estimation of the linear and angular velocities of the robot based on "encoders" and Inverse Kinematic model
